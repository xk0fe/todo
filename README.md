...
- Settings overlay (`s`): progress display, priority style, compact mode, task sorting, extensions
- `i` / `I` import/export the current project through its linked extension
...
**Extensions** — import/export tasks from external services through user-installable
extensions. An extension is any executable in `~/.todo/extensions` speaking a small
JSON-over-stdio protocol, so you can write your own in any language.
Linear, GitHub Issues and Trello extensions are bundled:
...
todo ext list
todo ext config linear api_key=KEY
todo ext link <space> <project> linear
todo ext import <space> <project>
todo ext export <space> <project>
```

See [docs/EXTENSIONS.md](docs/EXTENSIONS.md) for the protocol and how to write
your own extension.
